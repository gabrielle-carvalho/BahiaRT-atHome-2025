...
class VoiceNode(Node):
    def __init__(self):
        ...
        self.subscription_object_detected = self.create_subscription(String, '/detected_objects', self.self.detected_objects_callback, 10)
    ...
    def detected_objects_callback(self, msg):
        ...
        self.get_logger().info(f"Object detected: {object_name}")
        
        if object_name != 'Unknown':
            self.speak(object_name)
        else: 
            self.get_logger().info('No valid object detected')
    ...
    def listen_to_operator(self):
        try:
            with self.microphone as source:
                self.recognizer.adjust_for_ambient_noise(source)
                self.get_logger().info("Listening for operator...")
                audio = self.recognizer.listen(source)
                question = self.recognizer.recognize_google(audio).lower()
                self.get_logger().info(f"Operator said: {question}")
                
                if "qual é o objeto" in question:
                    self.awaiting_object = True
                    self.get_logger().info("Question detected. Waiting for object information...")
    ...
    def respond_to_operator(self):
        if self.detected_object:
            response = f"O objeto à minha frente é {self.detected_object}."
